...
# generation if an object type occurs in multiple parts.
    ...
    def transform(self, part):
        ''' Main function for the data transformation of part, from emdros raw data 
        up to LAF files
        '''
# open all files that need to be written to
        self.lf.start_annot(part)
        if part == 'monad':
            self.lf.start_primary()

# use existing or create new raw emdros data files
        self.wv.check_raw_files(part)

# the actual transformation
        self.process_lines(part)

# close all files that have been written
        self.lf.finish_annot(part)
        if part == 'monad':
            self.lf.finish_primary()
...
# fetch the monad limit
        monad_limit = self.cfg.args.limit

# make the config settings for this part readily available
        part_settings = self.cfg.parts[part]

# declare the file handles in local variables outside the loop
        region_handle = None
        plain_handle = None

# The monad part also creates the primary data.
# This is triggered by the config setting do_primary
        primary_handle = None
...
# The index data itself, once read in, is stored in the dictionary monad_chars.
        index_handle = None
        index_path = self.cfg.env['monad_index']
...
# initialize generation of primary data if needed
        if do_primary:
            ...
            region_handle = self.lf.file_handles[self.cfg.annotation_regions['name']][0]

        if separate_node_file:
            plain_handle = self.lf.file_handles[''][0]
...
# open the input file with raw data
        file_handle = codecs.open(self.wv.raw_file(part), encoding = 'utf-8')
...
# fetch template info
        ...
        region_types = self.cfg.annotation_regions
...
# fetch object list, subparts
        object_types = self.wv.object_list_part(part)
        subparts = sorted(self.lf.file_handles.keys())

# fetch additional information
        db_label = self.cfg.annotation_label['db_label']
        annotation_label = self.cfg.annotation_label[part + '_label']
        monad_type = self.cfg.parts['monad']['object_type']
...
# fetch additional feature info: what are the relational features?
        ref_features = {}
        for ref_feature in self.wv.list_ref_noskip():
            ref_features[ref_feature] = None
...
# by way of progress indication
            n_line += 1
            if n_line % 1000 == 0:
                sys.stderr.write("\rINFO: " + str(n_line))
                #if n_line >= 10000:
                #    break
...
# but only when the object type has changed!
            ...
            if prev_object_type != object_type:
                self.seen[prev_object_type] = True
                generate_nodes = object_type not in self.seen

                prev_features = {}

                prev_object_type = object_type
                this_subpart = self.wv.the_subpart(part, object_type)
...
# Maybe we have to stop:
            if monad_limit and int(maxmonad) > monad_limit:
                break
...
# So compute the length of the unicode strings that we need to write to the primary data
                ...
                #ltext = len(text.encode('utf-8'))
                ltext = len(text)
                #lsuffix = len(suffix.encode('utf-8'))
                lsuffix = len(suffix)
                n_m += 1
                if make_index:
            ...
            if separate_node_file and generate_nodes:
                ...
                if object_type == monad_type:
                    ...
                    n_data = node_elem.format(
                        part = part[0], akind = db_label, aid = object_id, xmlid = object_id,
                        monads = monads, minmonad = minmonad, maxmonad = maxmonad, objectid = object_id, objecttype=object_type,
                        region = " ".join(map(lambda x: "{}_{}".format(region_word, str(x)), regions))
                    ) 
                statsdb[''] += 1
                n_data += "\n"
                n_n += 1
...
# Construct REGIONS, NODES, ANNOTATIONS, EDGES if needed, per subpart
                if subpart == '' and do_primary:
                    continue
                if subpart == region_types['name']:
# this is a subpart where a file with only regions has been constructed. So we can skip adding regions to the annotation file.
                    continue
                features = {}
                if do_get_features:
                    features = self.wv.feature_list_subpart(part, subpart, object_type)
                    prev_features[subpart] = features
                else:
                    features = prev_features[subpart]
...
# This list will be sorted later
                if find_embedding:
                    sort_objects.append((min(regions), max(regions), object_type, object_id))
            ...
            for (outer, inner) in hierarchy:
                ...
                for (start, end, object_type, object_id) in sorted_objects:
                    ...
                    if object_type == inner:
                        if int(cur_outer_id) < 0:
                            print("ERROR: {} with id {} not inside any {}".format(inner, object_id, outer))
                        else:
                            n_en += 1
...
# Transfer the statistics information to the LAF object, so that the annotation files can be finalised properly
        for ob in stats:
            for sp in stats[ob]:
                self.lf.stats["{}.{}".format(sp, ob)] = stats[ob][sp]
...
# Fill in general overall statistics
        ...
        if n_unmatched:
            print("ERROR: {} Unmatched lines".format(n_unmatched))
        else:
            print("INFO: {} object lines".format(n_line))
